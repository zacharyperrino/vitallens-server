 ...
 * Call this after any data write for that user.
 */
...
function buildSupplementRecommendations(supplements, nutritionGaps, labGaps) {
    ...
    const results = [];
    const addMatch = (supplement, reason, currentlyTaking) => {
        results.push({ supplement, reason, currentlyTaking });
    };
...
export async function buildFullContext(userId, options = {}) {
    ...
    const [
        profileRes, dailyNutritionRes, mealsRes, sleepRes, exerciseRes,
        biomarkerScansRes, supplementsRes, environmentRes, labResultsRes,
        tcmProfileRes, todayNutritionRes,
    ] = await Promise.allSettled([
        supabase.from("health_profile").select("*").eq("user_id", userId).single(),
        supabase.from("daily_nutrition").select("date, calories, protein, carbs, fat, fiber").eq("user_id", userId).gte("date", dateStr(window)).order("date", { ascending: true }),
        supabase.from("meals").select("name, calories, protein, carbs, fat, fiber, logged_at").eq("user_id", userId).gte("logged_at", windowStart).order("logged_at", { ascending: false }).limit(50),
        supabase.from("sleep_log").select("hours, quality, bedtime, sleep_latency_min, wakeups, hrv_ms, date").eq("user_id", userId).gte("date", dateStr(window)).order("date", { ascending: true }),
        supabase.from("exercise_log").select("type, name, duration, intensity, rpe, muscle_groups, total_volume_kg, logged_at").eq("user_id", userId).gte("logged_at", windowStart).order("logged_at", { ascending: false }),
        ...
        supabase.from("environment_logs").select("location, aqi, aqi_category, pm2_5, uv_index, water_risk, logged_at").eq("user_id", userId).order("logged_at", { ascending: false }).limit(30),
    ...
    const exerciseData = exerciseRes.status === "fulfilled" ? exerciseRes.value.data || [] : [];
    ...
    const avgCalories = daysLogged > 0 ? Math.round(dailyNutrition.reduce((s, d) => s + d.calories, 0) / daysLogged) : 0;
    const avgProtein = daysLogged > 0 ? Math.round(dailyNutrition.reduce((s, d) => s + d.protein, 0) / daysLogged) : 0;
    const avgFiber = daysLogged > 0 ? Math.round(dailyNutrition.reduce((s, d) => s + d.fiber, 0) / daysLogged) : 0;
    const calorieTarget = profile?.target_calories || 2000;
    const proteinTarget = profile?.target_protein || 120;

    const sleepNights = sleepData.length;
    const avgSleepHours = sleepNights > 0 ? (sleepData.reduce((s, e) => s + (e.hours || 0), 0) / sleepNights).toFixed(1) : null;
    const avgSleepLatency = sleepData.filter(e => e.sleep_latency_min).length > 0
        ? Math.round(sleepData.filter(e => e.sleep_latency_min).reduce((s, e) => s + e.sleep_latency_min, 0) / sleepData.filter(e => e.sleep_latency_min).length) : null;
    const bedtimes = sleepData.filter(e => e.bedtime).map(e => { const [h, m] = e.bedtime.split(":").map(Number); return h * 60 + m; });
    ...
    const exerciseSessions = exerciseData.length;
    const totalExerciseMins = exerciseData.reduce((s, e) => s + (parseInt(e.duration) || 0), 0);
    const liftSessions = exerciseData.filter(e => e.type === "Weight Training").length;
    ...
    const snapshot = {
        ...
        sleep: { nightsLogged: sleepNights, avgHours: avgSleepHours, avgLatencyMin: avgSleepLatency, bedtimeConsistency, trend: trendVector(sleepData.map(e => e.hours || 0)), recentLog: sleepData.slice(-7) },
        exercise: { totalSessions: exerciseSessions, totalMinutes: totalExerciseMins, liftSessions, cardioSessions: exerciseSessions - liftSessions, recentSessions: exerciseData.slice(0, 7) },
...
export function snapshotToText(s) {
    ...
    const n = s.nutrition;
    t += "NUTRITION (" + n.daysLogged + " days): avg " + n.avgCalories + "cal (" + n.avgCalPct + "% target, " + n.calorieTrend + "), avg " + n.avgProtein + "g protein (" + n.avgProtPct + "%, " + n.proteinTrend + "), avg " + n.avgFiber + "g fiber\n\n";
    const sl = s.sleep;
    if (sl.nightsLogged > 0) t += "SLEEP (" + sl.nightsLogged + " nights): avg " + sl.avgHours + "h (" + sl.trend + "), latency " + (sl.avgLatencyMin || "?") + "min, bedtime " + sl.bedtimeConsistency + "\n\n";
    const ex = s.exercise;
    if (ex.totalSessions > 0) t += "EXERCISE: " + ex.totalSessions + " sessions (" + ex.liftSessions + " resistance, " + ex.cardioSessions + " cardio), " + ex.totalMinutes + " min total\n\n";
    ...
    if (s.supplements.gapAnalysis) {
        ...
        if (s.supplements.gapAnalysis.nutritionGaps.length > 0) {
            t += "Nutrition gaps: " + s.supplements.gapAnalysis.nutritionGaps.map(g => `${g.name} ${g.status} (${g.actual}${g.unit}/${g.target}${g.unit})`).join("; ") + "\n";
        }
        if (s.supplements.gapAnalysis.labGaps.length > 0) {
            t += "Lab flags: " + s.supplements.gapAnalysis.labGaps.map(g => `${g.marker} ${g.status}`).join("; ") + "\n";
        }
        if (s.supplements.gapAnalysis.interactionWarnings.length > 0) {
            t += "Supplement warnings: " + s.supplements.gapAnalysis.interactionWarnings.map(w => w.note).join("; ") + "\n";
        }
        t += "\n";
    }
